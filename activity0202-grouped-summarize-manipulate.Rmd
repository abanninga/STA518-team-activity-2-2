---
title: "Activity 2.2 - Exploratory Data Analysis: Grouped Summarizing and Manipulating"
author: Purple
date: 09/10/2019
output: 
  html_document: 
    keep_md: yes
---

### Load packages

```{r load-packages, message=FALSE}
library(tidyverse)
library(gapminder)
```

### Load data

```{r load-data, message=FALSE}
(my_gm <- gapminder)
```


### Exercise 1

| Variable      |Description    |
| ------------- |:-------------:| 
| Country       |Country of origin|
| Continent     |Continent country is in|
| Year          |Ranges from 1952 to 2007 in increments of 5 years|
| lifeExp       |Life expectancy at birth, in years|
| Pop           |The population|
| GPD per capita|Measured in US$, inflation-adjusted|
### Exercise 2
```{r}
my_gm %>% 
  mutate((gdp = pop * gdpPercap), gdpBillion = round(gdp/1000000000,2))

```
### Exercise 3
```{r}
us_tib <-  my_gm %>%
  filter(country == "United States")
## This is a semi-dangerous way to do this variable
## I'd prefer to join on year, but we haven't covered joins yet (Activity 2.4)
my_gm <-  my_gm %>%
  mutate(tmp = rep(us_tib$gdpPercap, nlevels(country)),
         gdpPercapRel = gdpPercap / tmp,
         tmp = NULL)

my_gm %>% 
  filter(country == "United States") %>% 
  select(country, year, gdpPercapRel)
```



Creates a dataset filtered by country (specifically, United States), calculates the GPD per capita for the United States, and subsequently calculates the relative GPD per Capita per year.

### Exercise 4

...

### Exercise 5

...

### Exercise 6

...

### Exercise 7

...

### Exercise 8

...

### Exercise 9

...
